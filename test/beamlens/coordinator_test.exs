defmodule Beamlens.CoordinatorTest do
  ...
  describe "max_iterations with running operators" do
    test "does not finish when max_iterations reached but operators still running" do
      ref = make_ref()
      parent = self()

      :telemetry.attach(
        {ref, :max_iterations_reached},
        [:beamlens, :coordinator, :max_iterations_reached],
        fn _event, _measurements, metadata, _ ->
          send(parent, {:telemetry, :max_iterations_reached, metadata})
        end,
        nil
      )

      {:ok, pid} = start_coordinator(max_iterations: 2)
      ...
      assert_receive {:telemetry, :max_iterations_reached, _}, 1000

      state = :sys.get_state(pid)

      assert state.status == :running,
             "Coordinator should remain running when operators are still active"

      assert map_size(state.running_operators) == 1

      Process.exit(fake_operator_pid, :kill)
      stop_coordinator(pid)
      :telemetry.detach({ref, :max_iterations_reached})
    end

    test "finishes when max_iterations reached and no operators running" do
      ref = make_ref()
      parent = self()
      ...
      assert state.status == :idle,
             "Coordinator should finish when no operators are running"
